...
- **Stockfish 18**：使用约 7 MB 的单线程 lite WASM，按需懒加载；持续提示默认开启，自走出第一步起逐着自动分析（开局首着不预跑），也可点击 💡、按 F1 触发。
- **与电脑对弈**：勾选后对方回合由 Stockfish 自动应对；引擎异常时自动切回手动对弈并提示。
- **电脑自弈**：勾选「电脑自己对弈」后双方均由 Stockfish 连续走子，将杀/逼和/可判和时自动停止；期间锁定人工输入，取消勾选随时接管。
- **落子过场动画**：电脑落子先以黄色箭头预告（1.2 秒），随后棋子以拖拽动效滑向目标格（1.8 秒）；遵循系统「减少动态效果」设置。
